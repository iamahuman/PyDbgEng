...
class IDebugEventCallbacksSink:
    ...
    def Exception(self, this, ExceptionCode, ExceptionFlags, ExceptionRecord,
    ...
    def CreateProcess(self, this, ImageFileHandle, Handle, BaseOffset,
    ...
    def LoadModule(self, this, ImageFileHandle, BaseOffset, ModuleSize,
...
class PyDbgEng(IDebugEventCallbacksSink):
    ...
    def Breakpoint(self, this, bp):
        ...
        if (self.event_callbacks_sink_intereset_mask
            ...
            ret = self.event_callbacks_sink.Breakpoint(
                self, bpParams.Offset, bpParams.Id, bpParams.BreakType,
                bpParams.ProcType, bpParams.Flags, bpParams.DataSize,
    ...
    def Exception(self, this, exception, firstChance):
        ...
        if (self.event_callbacks_sink_intereset_mask
            ...
            ret = self.event_callbacks_sink.Exception(
                self, exception.ExceptionCode, exception.ExceptionFlags,
                exception.ExceptionRecord, exception.ExceptionAddress,
                exception.NumberParameters, exception.ExceptionInformation[0],
    ...
    def CreateProcess(self, this, ImageFileHandle, Handle, BaseOffset,
        ...
        if (self.event_callbacks_sink_intereset_mask
            ...
            ret = self.event_callbacks_sink.CreateProcess(
    ...
    def LoadModule(self, this, ImageFileHandle, BaseOffset, ModuleSize,
        ...
        if (self.event_callbacks_sink_intereset_mask
            ...
            ret = self.event_callbacks_sink.LoadModule(self, ImageFileHandle,
    ...
    def execute(self, command):
        self.idebug_control.Execute(DbgEng.DEBUG_OUTCTL_THIS_CLIENT,
                                    Command=command,
                                    Flags=DbgEng.DEBUG_EXECUTE_ECHO)
    ...
    def bp_set(self,
        ...
        bp_params = self.idebug_control.AddBreakpoint(
            Type=DbgEng.DEBUG_BREAKPOINT_CODE, DesiredId=preferred_id)
    ...
    def fill_register_map(self):
        if len(self.register_index_map) == 0:
            ...
            for name in [
                ...
                self.register_index_map[
                    name] = self.idebug_registers.GetIndexByName(name)

    def get_register_value(self, register):
        self.fill_register_map()
        reg_value = self.idebug_registers.GetValue(
            self.register_index_map[register])
    ...
    def set_register_value(self, register, value):
        ...
        self.idebug_registers.SetValue(self.register_index_map[register],
                                       reg_value_ptr)
    ...
    def read_object_attributes(self, pobject_attributes):
        object_attributes_buffer = self.read_virtual_memory(
            pobject_attributes, SIZE_OF_OBJECT_ATTRIBUTES)
        root_handle = struct.unpack(
            "<I",
            object_attributes_buffer[OBJECT_ATTRIBUTES_OFFSET_TO_ROOT_HANDLE:
                                     OBJECT_ATTRIBUTES_OFFSET_TO_ROOT_HANDLE +
                                     4])[0]
        uni_struct_object_name_ptr = struct.unpack(
            "<I",
            object_attributes_buffer[OBJECT_ATTRIBUTES_OFFSET_TO_OBJECT_NAME:
                                     OBJECT_ATTRIBUTES_OFFSET_TO_OBJECT_NAME +
                                     4])[0]
    ...
    def get_stack_trace(self, frames_count):
        frames_buffer = create_string_buffer(frames_count *
                                             sizeof(DbgEng._DEBUG_STACK_FRAME))
        ...
        frames_list = []
        for address_of_frame_buffer in range(offset, offset + frames_count * stride, stride):
